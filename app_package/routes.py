...
def create():
    ...
    form=CreationForm()
    if form.validate_on_submit():
        fields=["_id","cid","name","acc_type","balance","mobile"]
        cust_id+=1
        values=[cust_id,form.cid.data,form.name.data,form.acctype.data,form.balance.data,form.mobile.data]
...
def withdraw():
    ...
    if form.validate_on_submit():
        ...
        if atype=="Priority" and new_bal<50000 or atype=="Ordinary" and new_bal<10000:
            flash("NOT POSSIBLE need to maintain balance for Priority customers 50,000/- & Ordinary customers 10,000/-")
